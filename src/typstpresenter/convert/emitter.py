...
def _styles_typ(palette: dict[str, str]) -> str:
    """Shared color helpers for flow-mode output (one copy per output
    directory, imported by every deck emitted there). Combine with native
    `*bold*` / `_italic_` markup as needed, e.g. `#red[*Warning*]`."""
    lines = [
        "// Shared color helpers -- generated by typstpresenter, edit freely.",
        "// Regenerated deterministically; local edits survive unless the",
        "// deck is re-converted into this same directory.",
    ]
    for rgb, name in palette.items():
        lines.append(f'#let {name}(body) = text(fill: rgb("#{rgb}"), body)')
    return "\n".join(lines) + "\n"
...
def emit_minimal(
    ...
    def write(scales: dict[int, float], markers: bool) -> None:
        # sentinel: gives the last slide's page span. Must land *inside*
        # that slide's own content -- appended as bare markup after a
        # bracket-scoped slide (title-slide, centered-slide) it does not
        # stay on that slide's last page, see flow_slide_markup's docstring.
        sentinel = (f"#place(hide(context metadata((s: {slide_count}, "
        ...
        if any(f"#{fn}[" in body_text for fn in COLOR_PALETTE.values()):
            ...
            styles_path.write_text(_styles_typ(COLOR_PALETTE), encoding="utf-8",
                                   newline="\n")
            header.append('#import "styles.typ": *')
        header += [
            "",
            "#show: simple-theme.with(",
