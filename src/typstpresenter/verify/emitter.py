...
def emit_touying(
    ...
    margin_top = 40.0 if minimal else 0.0
    margin_left = 40.0 if minimal else 0.0
    margin_cfg = f"margin: (top: {margin_top}pt, bottom: 20pt, left: {margin_left}pt, right: {margin_left}pt)" if minimal else "margin: 0pt"
        ...
        for slide_index, slide in enumerate(prs.slides):
            ...
            for shape, bbox in iter_flat_shapes(slide.shapes):
                ...
                if probes:
                    ...
                    body.append(
                        f"  #place(top + left, dx: {bbox.x - margin_left:.2f}pt, dy: {bbox.y - margin_top:.2f}pt, "
            ...
            if cetz_lines:
                ...
                body.append(
                    f"  #place(top + left, dx: {overhang_left + adjust_x - margin_left:.2f}pt, "
                    f"dy: {overhang_top + adjust_y - margin_top:.2f}pt, cetz.canvas(length: 1pt, {{"
                )
